    ...
    User.findOne({ token: token })
      ...
      .then((userData) => {
        ...
        if (userData) {
          ...
          Activity.findOne({ title: userData.sportPlayed[0].title }).then(
            (activityData) => {
              ...
              if (activityData) {
                ...
                fetch(`https://wttr.in/${userData.city}?format=j1&lang=fr`)
                  .then((r) => r.json())
                  .then((meteoData) => {
                    //stocker les données meteo
                    let meteoDesc =
                      meteoData.current_condition[0].lang_fr[0].value;

                    //reponse avec les données du "user", "level" et meteo
                    res.json({
                      result: true,
                      dataUser: userData,
                      dataLevel: activityLevel,
                      dataMeteo: meteoDesc,
                    });
                  })
                  .catch((e) => {
      ...
      if (sportData) {
        ...
        User.updateOne(
          { token: token },
          {
            ...
            form: { reason: reason, dayTime: dayTime },
            sportPlayed: [sportData._id],
...
router.post("/levelupdate", (req, res) => {
  ...
  if (checkBody(req.body, ["token", "sport", "xp", "subLevel", "level"])) {
    ...
    User.updateOne(
      ...
      {
        xp: xp,
        currentSubLevelID: subLevel,
        currentLevelID: level,
        stats: { nbSessions: sessions, totalTime: playTime },
      }
    ).then((modifiedUser) => {
      //verifier que l' element user a été bien modifié
      if (modifiedUser.modifiedCount > 0) {
        User.findOne({ token: token })
          .populate("sportPlayed")
          .then((userData) => {
            //verifier si le user modifié a été bien trouvé
            if (userData) {
              //rechercher l' activité choisi dans la collection "activities"
    ...
    Medal.find().then((medalData) => {
      ...
      Activity.findOne({ title: levelTitle }).then((levelData) => {
        ...
        let testUser = new User({
          ...
          coordinate: {
            name: coordName,
            location: { type: coordType, coordinates: [coordLat, coordLon] },
          },
          city: city,
          notificationActive: notificationActive,
          photoUrl: photoUrl,
